    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 rounded-(--vm-radius-md) border border-(--vm-border) bg-(--vm-surface-2)/50 p-4">
        ...
        <div className="min-w-0">
          <p className="text-xs text-(--vm-muted)">Verification code sent to</p>
          <p className="mt-1 truncate text-sm font-semibold text-(--vm-text)">{email}</p>
        </div>
      </div>

      {/* Error State */}
      {error && (
        <div role="alert" className="rounded-(--vm-radius-md) border border-(--vm-danger)/30 bg-(--vm-danger)/10 px-4 py-3 text-sm text-(--vm-danger)">
          {error}
        </div>
      )}
      ...
      {/* Success State */}
      {success && (
        <div role="status" className="flex items-start gap-2 rounded-(--vm-radius-md) border border-(--vm-primary)/30 bg-(--vm-primary)/10 px-4 py-3 text-sm text-(--vm-text)">
          <CheckCircle2 size={17} className="mt-0.5 shrink-0 text-(--vm-primary)" />
          <span>{success}</span>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <div className="flex justify-center gap-2 sm:gap-3" aria-label="One-time password">
            {otp.map((digit, index) => (
              <input
                key={index}
        ...
        >
          {isVerifyingOtp && <Loader2 size={17} className="animate-spin" aria-hidden="true" />}
      ...
      <div className="text-center">
        ...
        <button
          ...
          className="mt-2 inline-flex cursor-pointer items-center gap-2 text-sm font-semibold text-(--vm-primary) transition-colors hover:text-(--vm-primary-pressed) disabled:cursor-not-allowed disabled:opacity-50"
        >
          <RefreshCw size={15} className={isSendingOtp ? "animate-spin" : ""} aria-hidden="true" />
      ...
      <button
        ...
        className="mx-auto flex cursor-pointer items-center gap-2 text-sm text-(--vm-muted) transition-colors hover:text-(--vm-text)"
      >
